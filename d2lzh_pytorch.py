    ...
    with torch.no_grad():
        for X, y in data_iter:
            if isinstance(net, nn.Module):
                net.eval()  # evaluation mode, which will close dropout.
                acc_sum += (net(X.to(device)).argmax(dim=1) == y.to(device)).float().sum().cpu().item()
                net.train()  # return to train mode
...
def semilogy(x_vals, y_vals, x_label, y_label, x2_vals=None, y2_vals=None, legend=None, figsize=(3.5, 2.5)):
    ...
    if x2_vals and y2_vals:
        ...
        plt.legend(legend)
    plt.savefig("/home/zhanghongtong/11.jpg")
...
def data_iter_random(corpus_indices, batch_size, num_steps, device=None):
    # 减1是因为输出的索引是相应输⼊的索引加1，`想``要``有``直``升` -> `要``有``直``升``机`
    ...
    for epoch in range(num_epochs):
        ...
        for X, Y in data_iter:
            if is_random_iter:  # 如使⽤随机采样，在每个⼩批量更新前初始化隐藏状态
                ...
                # 使模型参数的梯度计算只依赖一次迭代读取的⼩批量序列(防⽌梯度计算开销太大)
                for s in state:
                    s.detach_()
...
def train_and_predict_rnn_pytorch(model, num_hiddens, vocab_size, device, corpus_indices,
    ...
    for epoch in range(num_epochs):
        ...
        try:
            perplexity = math.exp(l_sum / n)  # 困惑度
        ...
        if (epoch + 1) % pred_period == 0:
            ...
            for prefix in prefixes:
                print(' -', predict_rnn_pytorch(prefix, pred_len, model, vocab_size,
