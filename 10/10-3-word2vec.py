...
# print('# tokens: %d' % sum([len(st) for st in subsampled_dataset]))  # '# tokens: 375875'
    ...
    for contexts in all_contexts:
        ...
        while len(negatives) < len(contexts) * K:
            if i == len(neg_candidates):
                ...
                # 作为噪声词candidates。为了高效计算，可以将k设得稍大一点。
...
# 指定 DataLoader 实例中⼩批量的读取⽅式
def batchify(data):
    """⽤作DataLoader的参数collate_fn: 输⼊是个⻓为batchsize的list,
    list中的每个元素都是Dataset类调⽤__getitem__得到的结果
    """
    ...
    for epoch in range(num_epochs):
        ...
        for batch in data_iter:
            ...
            l = (loss(pred.view(label.shape), label, mask) *
                 mask.shape[1] / mask.float().sum(dim=1)).mean()  # ⼀个batch的平均loss
            optimizer.zero_grad()
            l.backward()
            optimizer.step()
            l_sum += l.cpu().item()
